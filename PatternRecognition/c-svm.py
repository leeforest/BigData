...
class c_support_vector_machine:
	...
	def get_data(self):
		with open('datasets/dataset4/tr_x.txt','r') as f:
		...
		with open('datasets/dataset4/te_t.txt','r') as f:
	...
	def CSVM(self):
		...
		result=svc.fit(self.X_tr,self.Y_tr)

		#train data classification plot(with dicision surface,support vector)
		plt.figure(3)
		plt.title('C-SVM-train data Classification')
		predict1=svc.predict(self.X_tr)
		...
		Z=svc.decision_function(xy).reshape(X.shape)
		plt.contour(X,Y,Z,colors='k',levels=[-1,0,1],alpha=0.5,linestypes=['--','-','--'])
		plt.scatter(svc.support_vectors_[:,0],svc.support_vectors_[:,1],facecolors='yellow',edgecolor='k')

		#test data classification plot(with dicision surface,support vector)
		plt.figure(4)
		plt.title('C-SVM-test data Classification')
		predict2=svc.predict(self.X_te)
